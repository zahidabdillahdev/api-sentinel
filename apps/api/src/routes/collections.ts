...
import { resolveVariables } from '../lib/variables.js';
...
const collectionBody = z.object({ name: z.string().min(2).max(100), environmentId: z.string().cuid().optional() });
...
export const collectionRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/projects/:projectId/collections', { preHandler: app.authenticate }, async (request, reply) => {
    ...
    if (body.environmentId && !await app.prisma.environment.findFirst({ where: { id: body.environmentId, projectId } })) throw notFound('Environment');
    return reply.code(201).send(await app.prisma.collection.create({ data: { projectId, ...body } }));
  });
