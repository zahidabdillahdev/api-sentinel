...
describe('safe target URLs', () => {
  it('rejects local and non-HTTPS targets before making DNS requests', async () => {
    await expect(assertSafeTarget('ftp://example.com')).rejects.toThrow('Only public HTTP or HTTPS');
    await expect(assertSafeTarget('https://localhost/test')).rejects.toThrow('Only public HTTP or HTTPS');
